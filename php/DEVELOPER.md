...
```bash
sudo apt update -y
sudo apt install -y php-dev php-cli git gcc make autotools-dev libtool pkg-config openssl libssl-dev unzip
# Install rust
...
Development on the PHP wrapper may involve changes in either the C or PHP code. Each language has distinct linter tests that must be passed before committing changes.
...
# Check PHP code style (if you have PHP files)
phpcs --standard=PSR12 tests/
```

#### Fixing lint formatting errors

The following command can be used to fix C formatting errors:
