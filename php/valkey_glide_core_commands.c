...
static int parse_info_multi_node_response(CommandResult *cmd_result, zval *return_value)
{
    ...
    {
        zval temp_result;
        if (command_response_to_zval(cmd_result->response, &temp_result, COMMAND_RESPONSE_NOT_ASSOSIATIVE, false))
        {
    ...
    if (cmd_result->response)
    {
...

        {
            ...
            case String:
                ...
                command_response_to_zval(cmd_result->response, return_value, COMMAND_RESPONSE_NOT_ASSOSIATIVE, false);
                ret_val = 1;
                break;
